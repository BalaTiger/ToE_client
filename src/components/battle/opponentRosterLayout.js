...
export function getOpponentRosterLayout({ count, currentTurn, expandedPid = null, width, fullWidth = 154, compactWidth = 64 }) {
  ...
  const seats = widths.map((seatWidth, index) => {
    ...
    return { x, y: 0, width: seatWidth };
  });
  return { seats, height: Math.max(118, fullWidth * 390 / 458) };
}
