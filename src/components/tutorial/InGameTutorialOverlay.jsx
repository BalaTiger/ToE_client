...
import { getTutorialStep } from '../../game';
...
function NarratorAvatar({tooltipW}){
  const sz=Math.max(36,Math.min(58,Math.floor((tooltipW||260)/5)));
  return(
...
const TERM_STYLES = {
  ...
  邪祀者: { color: '#BA9BCB', fontStyle: 'normal', fontWeight: 700, textShadow: '0 0 8px #7040aa88' },
  ...
  蛊惑: { color: '#BA9BCB', fontStyle: 'normal', fontWeight: 700, textShadow: '0 0 8px #7040aa88' },
  ...
  SAN: { color: '#7bb9bd', fontStyle: 'normal', fontWeight: 700, textShadow: '0 0 8px #326e7288' },
};
...
function getScriptHighlightRect(step, rects) {
  ...
  switch (step?.highlight) {
    ...
    case 'handCard':
      return rects.tutorialHandCardRect;
    case 'handCards':
      return rects.handCardsRect || rects.handAreaRect;
    case 'skillButton':
      return rects.skillButtonRect;
    case 'opponentPanel':
...
function ScriptTutorialOverlay({
  tutorialStep,
  vw,
  panelRect,
  roleTextRect,
  ...
  const revealCard = ['drawRevealKeepButton', 'godKeepHandButton'].includes(step?.highlight)
    ? _getZoomCompensatedRect(document.querySelector('[data-card-reveal]')) : null;
  const tooltipPosition = getTutorialTooltipPosition({
    rect, width: tooltipW, height: tooltipHeight || 210 * tm.sizeScale,
    vw: window.innerWidth, vh: window.innerHeight,
    avoid: revealCard ? [revealCard] : [], centered: step?.highlight === 'center' || noSpotlight,
  });
  if (!step || step.auto || (actionStep && !rect)) return null;
  ...
  return (
    <div data-tutorial-overlay data-tutorial-step={tutorialStep} style={{ position: 'fixed', inset: 0, zIndex: 1100, pointerEvents: actionStep ? 'none' : 'auto' }}>
      {!noSpotlight && (r ? (
        <>
          ...
          <div style={{ position: 'absolute', right: 0, top: r.top, bottom: 0, left: r.right, background: BG, pointerEvents: 'none' }} />
          <div style={{ position: 'absolute', left: r.left, right: window.innerWidth - r.right, top: r.bottom, bottom: 0, background: BG, pointerEvents: 'none' }} />
          <div style={{ position: 'absolute', left: r.left, top: r.top, width: r.right - r.left, height: r.bottom - r.top, border: '1.5px solid #c8a96e', boxShadow: '0 0 20px #c8a96e66, inset 0 0 18px #c8a96e22', borderRadius: 4, pointerEvents: 'none' }} />
        </>
      ) : (
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.72)', pointerEvents: 'none' }} />
      ))}
      <div ref={tooltipRef} className="toe-dialog toe-dialog--tutorial" data-ui-dialog="tutorial" role="dialog" aria-label={step.title || '新手引导'} style={{
        position: 'absolute',
        ...tooltipPosition,
        width: tooltipW,
...
export default function InGameTutorialOverlay({ showTutorial, ...props }) {
  if (!showTutorial || typeof props.tutorialStep !== 'string') return null;
  return <ScriptTutorialOverlay {...props} />;
}
