...
export function GammaSlider({ gamma, onChange, musicVolume=1, onMusicVolumeChange, sfxVolume=1, onSfxVolumeChange, defaultOpen=false, battleControls }) {
  ...
  return createPortal(<div ref={panelRef} className={`toe-settings${docked ? ' toe-settings-coastal' : ''}`} style={battleControls?.style} onMouseEnter={()=>{if(!docked)setHover(true);}} onMouseLeave={()=>setHover(false)} onKeyDown={e=>{if(e.key==='Escape'){close();triggerRef.current?.focus();}}}>
    <div className="toe-settings-triggers">
      {docked && !battleControls.isMultiplayer && <button className="toe-button toe-coastal-system-button" type="button" aria-label="暂停游戏" title={battleControls.showTutorial ? '教学中不可暂停' : '暂停游戏'} disabled={battleControls.showTutorial} onClick={battleControls.onPause}><img src={buildPublicUrl('/img/ui/coastal/system-pause.webp')} alt="" aria-hidden="true" draggable={false} /></button>}
      <button ref={triggerRef} className={`toe-button toe-settings-toggle${docked ? ' toe-coastal-system-button' : ''}`} aria-label={docked ? '对局菜单' : '视听设置'} title={docked ? '对局菜单' : '视听设置'} aria-expanded={open} onClick={()=>setPinned(v=>!v)}>{docked ? <img src={buildPublicUrl('/img/ui/coastal/system-settings.webp')} alt="" aria-hidden="true" draggable={false} /> : '☀ / ♪'}</button>
    </div>
