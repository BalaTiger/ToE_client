...
export function GammaSlider({ gamma, onChange, musicVolume=1, onMusicVolumeChange, sfxVolume=1, onSfxVolumeChange, defaultOpen=false, battleControls, startControlScale }) {
  ...
  useEffect(() => {
    ...
    const dismiss = event => {
      if (!panelRef.current?.contains(event.target) && !popupRef.current?.contains(event.target)) {
  ...
  const panel = open && <section ref={popupRef} className="toe-dialog toe-settings-panel" aria-label="视听设置面板" style={docked ? { pointerEvents: 'auto' } : undefined}>
  ...
  const popup = docked && panel ? renderGameLayer(
    <div className="toe-settings toe-settings-coastal" data-settings-popup style={{
      ...battleControls.style,
      // Keep the existing unshaken anchor. The empty trigger-height slot must
      // not intercept clicks on the actual controls below the overlay layer.
      paddingTop: 54 * (battleControls.style?.['--toe-coastal-system-scale'] ?? 1),
      pointerEvents: 'none',
    }}>{panel}</div>,
  ) : panel;
