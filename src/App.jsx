...
import { buildBewitchGiftReplay } from './game/animReplayEvents';
...
const AUTHORITATIVE_QUEUE_META = Object.freeze({
  authority: ANIMATION_QUEUE_AUTHORITY.QUEUE,
});
const LEGACY_MERGE_ACTION_SCOPE_META = Object.freeze({
  authority: ANIMATION_QUEUE_AUTHORITY.LEGACY_MERGE,
  visualEventScope: 'action',
});

function authoritativeTurnStartQueueMeta(state) {
...
function buildTurnStartIntroQueue(state,name){
  ...
  queue.push({type:'YOUR_TURN',turnStartStage:TURN_START_ANIMATION_STAGE.TURN_BANNER,name:name||state.players?.[state.currentTurn]?.name||'???',msgs:state._turnStartLogs});
...
export default function Game(){
  ...
  const finishTutorialActionWithState=useCallback((nextGs,nextStep,queue=[],transactionMeta=null)=>{
    if(!nextStep){
      if(queue?.length)triggerAnimQueue(queue,nextGs,undefined,transactionMeta);
      else setGs(nextGs);
      return;
    }
    ...
    if(queue?.length){
      ...
      triggerAnimQueue(queue,finalTutorialGs,completeStep,transactionMeta);
    }else{
  ...
  useEffect(()=>{
    ...
    const finishTimeoutTurn=(resolvedBase)=>{
      ...
      if(resolvedBase.players[0].hand.length>baseHandLimit){
        setGs({...resolvedBase,phase:'DISCARD_PHASE',_turnFlowStage:TURN_FLOW_STAGE.DISCARD,abilityData:{discardSelected:[],fromEndTurn:true}});
        return;
      }
    ...
    if(shouldAnimateDrawDiscard){
      ...
      if(base.players[0].hand.length>baseHandLimit){
        triggerAnimQueue([{type:'DISCARD',card:dr.card,triggerName:who,targetPid:drawerIdx,msgs:[discardMsg]}],{...base,phase:'DISCARD_PHASE',_turnFlowStage:TURN_FLOW_STAGE.DISCARD,abilityData:{discardSelected:[],fromEndTurn:true}},undefined,AUTHORITATIVE_QUEUE_META);
        return;
      }
...
{isLose?(winner==='LOSE_ALL'?'——  全员覆灭  ——':'英魂殒落'):iWon?'胜利归你':winner===ROLE_TREASURE?`——  ${gs.players[winnerIdx]?.name??''}获胜  ——`:'——  '+winner+'获胜  ——'}
  ...
  function _cthContinueRestDraws(baseGsAfterDecision){
    ...
    const dreamSteps=(msgs=[])=>playDream?[makeCthRlyehDreamStep(0,msgs)]:[];
    ...
    if(baseGsAfterDecision._cthRestDraws?.length>0){
      ...
      triggerAnimQueue(
        [...dreamSteps(baseGsAfterDecision._cthRestDrawLogs||[]),...cthQueue,...statQ,statePatchStep({players:cleanedGs.players,discard:cleanedGs.discard})],
        null,
    ...
    for(let _d=0;_d<remaining;_d++){
      ...
      if(!baseGsAfterDecision.geomagneticReversalActive){
        ...
        if(zhuGuard){
          ...
          triggerAnimQueue(dreamSteps(L.slice(-1)),afterDreamGs);
          return;
        }
      }
      ...
      if(r2.needGodChoice){
        ...
        triggerAnimQueue([...dreamSteps(newGs._drawLogs),...queue],newGs);
        return;
      }
      if(r2.needsDecision){
        ...
        triggerAnimQueue([...dreamSteps(drawMsg?[drawMsg]:[]),{type:'DRAW_CARD',card:r2.drawnCard,triggerName:'你',targetPid:0,msgs:drawMsg?[drawMsg]:[]}],newGs);
        return;
      }
      ...
      if(r2.kept){
        ...
        if(forcedGs._isMP)broadcastAnimTransaction(forcedGs,queue,{context:'cthRlyehDream',barrier:'continuation',msgs:L.slice(baseGsAfterDecision.log.length),beforePlayers:baseGsAfterDecision.players,beforeDiscard:baseGsAfterDecision.discard});
        triggerAnimQueue(
          queue,
          null,
          ()=>{
  ...
  // A decision reached while an end-turn replay is open is still part of the
  // same visible timeline. Publish its exact local queue before opening the
  // decision/continuation state remotely, instead of asking the remote client
  // to infer timing from a state diff.
  function broadcastEndTurnDecisionAnimTransaction(state,steps=[],msgs=[]){
    if(!state?._isMP||!endTurnReplaySyncQueueRef.current)return false;
  ...
  function advanceEndTurnSeq(state){
    const seq=endTurnSeqRef.current;
    if(!seq){applyNextTurnGs(startNextTurn({...state,_turnFlowStage:null}));return false;} // 非调度路径兜底
  ...
  function finishEndTurnSeq(state){
    ...
    const nextGs=withEndTurnReplaySyncEvent(startNextTurn({...state,_turnFlowStage:null}));
    applyNextTurnGs(nextGs);
    return true;
  }
  ...
  function kickoffEndTurnSeq(baseGs,{seedQueue=[],preStatePatch={}}={}){
    ...
    const seqState=withClearedTurnAnimFields({...baseGs,...preStatePatch,_turnFlowStage:TURN_FLOW_STAGE.END_TURN});
    if(seedQueue.length){
  ...
  function handleDrawKeep(){
    ...
    if(hasEffectDecisionState(res.statePatch)){
      ...
      if(decisionQueue.length){
        ...
        triggerAnimQueue(decisionQueue,decisionGs);
      }else{
        syncVisibleLog(L);
        setGs(decisionGs);
      }
  ...
  function handleTreasureDodge(gs, dr, isAOE = false) {
    ...
    const fallbackAbilityData={fromRest:gs.abilityData?.fromRest,fromTsathogguaSlime:gs.abilityData?.fromTsathogguaSlime,continueTurnStartDraw:gs.abilityData?.continueTurnStartDraw,cthDrawsRemaining:gs.abilityData?.cthDrawsRemaining,_turnOwner:gs.abilityData?._turnOwner};
    const newGs = {
      ...gs,
      players: P,
      deck: D,
  ...
  function handleTreasureDodgeRollMode(aoe=false){
    ...
    if(flowKind==='pendingWin'){
      ...
      setGs(p=>p?{...p,phase:'ACTION',drawReveal:null}:p);
      triggerAnimQueue(queue,result.pendingWinGs);
      return;
    }
    if(flowKind==='rest'){
      // 播放骰子动画后再处理剩余摸牌
    ...
    if(flowKind==='slime'){
      ...
      setGs(p=>p?{...p,phase:'ACTION',drawReveal:null}:p);
      triggerAnimQueue([diceAnim,...queue],result.newGs,()=>_tsgContinueTurnStartDraw(result.newGs));
      return;
    }
    const queue=bindAnimLogChunks(buildAnimQueue(gs,result.newGs),splitAnimBoundLogs(result.L.slice(gs.log.length)));
    const drawerIdx=getTreasureDodgeDrawerIdx(gs,dr,aoe);
  ...
  function handleTreasureDodgeSkipMode(aoe=false){
    ...
    if(!dr.fromEndTurnReplay)P[drawerIdx].hand.push(resolutionCard);
    const who=localDisplayName(drawerIdx,P[drawerIdx].name);
    const skipLog=aoe?'你选择不规避负面效果':`${who} 收入了 ${cardLogText(resolutionCard,{alwaysShowName:true})}`;
    ...
    const decisionState=config.deriveSkipDecision?deriveEffectDecisionState(res.statePatch,{
      baseAbilityData:gs.abilityData,
      fallbackPhase:'ACTION',
      extraAbilityData:{
        ...(dr.fromRest?{fromRest:true}:{}),
    ...
    const newGs={...gs,players:P,deck:D,discard:Disc,log:L,phase:decisionState.hasDecision?decisionState.phase:'ACTION',drawReveal:null,abilityData:decisionState.hasDecision?decisionState.abilityData:fallbackAbilityData,...(config.deriveSkipDecision?(res.statePatch||{}):{}),...replayPatch};
    if(decisionState.hasDecision){
      newGs.phase=decisionState.phase;
      newGs.abilityData=decisionState.abilityData;
    }
...
const L=[...baseLog,`【两人一绳】${sourcePlayer.name} 与 ${targetPlayer.name} 间架起链条，一方受到HP伤害时另一方受等量伤害`];
  ...
  function roseThornSelectTarget(ti){
    ...
    if(ti!==0&&!P[ti].isDead&&P[ti].role===ROLE_TREASURE&&isWinHand(P[ti].hand)){
      ...
      triggerSyncedAnimTransaction(queue,winGs,{context:'roseThorn',barrier:'gameOver',msgs:L.slice(gs.log.length),beforePlayers:gs.players,beforeDiscard:gs.discard});
      return;
    }
    const nextGs={...buildTargetContinuationGs({players:P,deck:D,discard:Disc,log:L}),...apophisNightPatch(night)};
  ...
  async function igniteTorchDiscardCard(cardIndex, allowAi=false, baseState=null){
    ...
    if(damageDecision?.phase)nextGs={...nextGs,phase:damageDecision.phase,abilityData:damageDecision.abilityData};
    const effectQueue=[{type:'DISCARD',card:discardedCard,targetPid:actorIdx,msgs:L.slice(-2)},statePatchStep({players:P,discard:Disc,log:L})];
    if(nextGs._isMP)broadcastAnimTransaction(nextGs,effectQueue,{context:'cardEffectDiscard',barrier:nextGs.phase==='ACTION'?'continuation':'decision',msgs:L.slice(gs.log.length),beforePlayers:gs.players,beforeDiscard:gs.discard});
    finishTargetContinuation({
      queue:effectQueue,
      nextGs,
      continueRest:!!(abilityData.fromRest&&isLocalSeatIndex(actorIdx)),
  ...
  function resolveSharedBuryAlive(state,autoFill=false){
    ...
    const nextGs=buildTargetContinuationGs({players:P,deck:D,discard:Disc,log:L,turnOwner,abilityData});
    if(queue.length)broadcastAnimTransaction(nextGs,queue,{context:'buryAlive',barrier:'continuation',msgs:L.slice(state.log.length),beforePlayers:state.players,beforeDiscard:state.discard});
    finishTargetContinuation({
      queue,
      nextGs,
      continueRest:!!(abilityData.fromRest&&isLocalSeatIndex(abilityData.source)),
  ...
  function huntSelectTarget(ti){
    ...
    if(gs._isMP){
      ...
      const queue=mergeApophisTargetQueue([{type:'SKILL_HUNT',targetIdx:ti,msgs:huntMsgs}],gs,huntWaitGsWithEvent);
  ...
  function buildPostBewitchStageQueues(oldGs,encounterGs,newGs){
    if(!encounterGs){
      const acceptanceQueue=buildPostBewitchStatQueue(oldGs,newGs);
      return {encounterQueue:[],acceptanceQueue,queue:acceptanceQueue};
    }
    const encounterQueue=buildPostBewitchStatQueue(oldGs,encounterGs);
    const acceptanceQueue=buildPostBewitchStatQueue(encounterGs,newGs);
    return {
      encounterQueue,
      acceptanceQueue,
      queue:[...encounterQueue,...acceptanceQueue],
    };
  }
  ...
  function bewitchSelectTarget(ti){
    ...
    if(bewitchCard.isGod){
      ...
      const newGs=clearTurnDrawReplayHints({...gs,players:P,deck:D,discard:Disc,log:L,drawReveal:null,skillUsed:true,...mergedInspectionMeta,...nightPatch,...(gres.statePatch||{}),phase:nextPhase,abilityData:nextAbilityData,apophisNight:nextApophisNight,_visualEvents:[...(gres.statePatch?._visualEvents||[])],...(win?{gameOver:win}:{})});
      const stageQueues=buildPostBewitchStageQueues(gs,encounterGs,newGs);
      const queue=mergeApophisTargetQueue(buildBewitchForcedCardQueue(0,ti,bewitchCard,P[ti]?.name,stageQueues.queue,bewitchMsgs,{
        playersAfter:P,
        zhuLightBefore:gs.zhuLight||null,
        zhuLightAfter:newGs.zhuLight||null,
        encounterQueue:stageQueues.encounterQueue,
        acceptanceQueue:stageQueues.acceptanceQueue,
      }),gs,newGs);
    ...
    const newGs=clearTurnDrawReplayHints({...gs,players:res.P,deck:res.D,discard:res.Disc,log:L,drawReveal:null,
      ...
      phase:nextPhase,
      skillUsed:true,...(res.statePatch||{}),...apophisNightPatch(night),_visualEvents:[...(res.statePatch?._visualEvents||[])],...(win?{gameOver:win}:{})});
      const stageQueues=buildPostBewitchStageQueues(gs,null,newGs);
      const queue=mergeApophisTargetQueue(buildBewitchForcedCardQueue(0,ti,bewitchCard,res.P[ti]?.name,stageQueues.queue,bewitchMsgs,{
        playersAfter:res.P,
        acceptanceQueue:stageQueues.acceptanceQueue,
      }),gs,newGs);
      broadcastAnimTransaction(newGs,queue,{context:'bewitchGift',barrier:nextPhase==='ACTION'?'continuation':'decision',msgs:bewitchMsgs,beforePlayers:gs.players,beforeDiscard:gs.discard});
  ...
  function buildQueuedNextAiTurnStartReplay(nextGs,{fromTurn=null,playersBeforeDraw=null,statEventSeq=null}={}){
    ...
    maskDiscardedTurnDrawUntilDiscardAnim(replayBaseGs);
  ...
  function normalizeVisibleTurnStartQueue(queue=[]){
    ...
    const turnIdx=combinedStartQueue.findIndex(step=>step?.type==='YOUR_TURN');
    if(turnIdx<=0)return [...turnBoundaryQueue,...combinedStartQueue,...drawQueue];
    const leadingLocks=combinedStartQueue.slice(0,turnIdx).filter(step=>step?.type==='VISUAL_LOCK');
    const visibleBeforeTurn=combinedStartQueue.slice(0,turnIdx).filter(step=>step?.type!=='VISUAL_LOCK');
    return [
      ...turnBoundaryQueue,
      ...leadingLocks,
      combinedStartQueue[turnIdx],
      ...visibleBeforeTurn,
      ...combinedStartQueue.slice(turnIdx+1),
      ...drawQueue,
    ];
  }
  ...
  function buildCthRestDrawReplayEvent({beforePlayers,beforeDiscard,zhuLight,actorName,cthDraws,cthDrawLogs,preSteps=[],statSteps=[],playDream=true}){
    ...
    return createAnimTransactionEvent({
      ...
      queue:[
        ...
        ...(playDream?[makeCthRlyehDreamStep(0,logs)]:[]),
        ...drawSteps,
