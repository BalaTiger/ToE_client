...
SRC = ROOT / "public/img/card/cardback_stars_call.png"
OUT_DIR = ROOT / "public/img/card/animated/stars_call_comfy_refraction"
SIZE = (392, 590)

PROMPT = (
    "preserve the exact game card back composition, border, silhouette and crop. "
    "Only reinterpret the internal ocean-and-stars texture inside the card back: "
    "underwater caustic refraction, rippling water lens distortion, abyssal blue starlight, "
    "large readable wave refraction bands, subtle cosmic shimmer, no text, no logo, "
    "no extra symbols, no border change, no camera movement"
)
...
def upload_image(path: Path, name: str):
    ...
    fields = [
        ...
        (b"type", b'Content-Disposition: form-data; name="type"\r\n\r\ninput\r\n'),
        (b"overwrite", b'Content-Disposition: form-data; name="overwrite"\r\n\r\ntrue\r\n'),
    ]
    body = io.BytesIO()
...
def make_workflow(image_name, seed):
    return {
        ...
        "2": {"class_type": "ModelPatchLoader", "inputs": {"name": "Z-Image-Turbo-Fun-Controlnet-Tile-2.1-lite-2601-8steps.safetensors"}},
        ...
        "6": {"class_type": "Canny", "inputs": {"image": ["5", 0], "low_threshold": 0.03, "high_threshold": 0.18}},
        "7": {"class_type": "QwenImageDiffsynthControlnet", "inputs": {"model": ["1", 0], "model_patch": ["2", 0], "vae": ["3", 0], "image": ["6", 0], "strength": 0.58}},
        ...
        "16": {"class_type": "SaveImage", "inputs": {"images": ["15", 0], "filename_prefix": "stars_cardback_refraction"}},
    }
...
def composite_candidate(candidate: Image.Image, base: Image.Image, mask: Image.Image):
    ...
    mixed = Image.blend(base, candidate, 0.48)
...
def main():
    ...
    input_path = OUT_DIR / "stars_cardback_input.png"
    base.save(input_path)
    mask = make_inner_mask()
    mask.save(OUT_DIR / "stars_cardback_inner_mask.png")
    image_name = upload_image(input_path, "stars_cardback_input.png")
    client_id = str(uuid.uuid4())
    outputs = []
    for i, seed in enumerate([930240, 930241, 930242]):
        ...
        raw_path = OUT_DIR / f"stars_cardback_refraction_raw_{i:02d}.png"
        raw.save(raw_path)
        comp = composite_candidate(raw, base, mask)
        comp_path = OUT_DIR / f"stars_cardback_refraction_comp_{i:02d}.png"
        comp.save(comp_path)
