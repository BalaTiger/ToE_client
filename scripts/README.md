...
- `simulate-headless.mjs` - runs repeated games through the shared rule/AI modules and validates staged turn-start presentation queues for cross-turn stat replay
...
- `npm run sim:headless` - run `simulate-headless.mjs`; any rule-flow or presentation-ownership failure exits non-zero
