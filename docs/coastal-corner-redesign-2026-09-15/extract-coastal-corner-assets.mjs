...
const docs = path.join(root, 'docs/coastal-corner-redesign-2026-09-15');
...
const sources = [
  { name: 'turn-plaque', file: 'exec-1acdbf70-3a1e-4ea1-9c91-b21e914aad17.png', matteMinChannel: 110 },
  { name: 'log-journal', file: 'exec-8eea696b-0617-4461-ac8e-46ff05b6d486.png' },
  { name: 'control-disc', file: 'exec-36024573-7058-4de1-9b0e-49222d43eb4a.png', square: true },
];
...
function extractAlpha(rgb, width, height, matteMinChannel = 130) {
...
for (const source of sources) {
  ...
  const { rgba, trim, alphaBounds, clearedComponents } = extractAlpha(rgb, info.width, info.height, source.matteMinChannel);
  ...
  records.push({
    ...
    sourceSize: [info.width, info.height], sourceSha256: createHash('sha256').update(original).digest('hex'),
    alphaBounds, trim, extension, matteMinChannel: source.matteMinChannel ?? 130,
...
await fs.copyFile(script, path.join(docs, 'extract-coastal-corner-assets.mjs'));
await fs.writeFile(path.join(docs, 'production-assets.json'), `${JSON.stringify({
  generator: 'scripts/extract-coastal-corner-assets.mjs',
  ...
  processing: {
    ...
    neutralMatte: { minChannel: '130 by default; 110 for the darker turn-plaque checkerboard', maxChannelSpread: 22 },
    enclosedCheckerboard: { minArea: 32, minLuminanceRange: 38 },
    alpha: 'Exterior/checked-hole removal, largest connected subject, one-pixel edge decontamination',
